...
- Treat those files as application facts. Browsing, collecting, saving, or researching a role never means it was applied to.
...
- After the opportunity search, add only newly verified, currently live official job URLs to the `data/pipeline.md` Pending section using Career Ops' canonical `- [ ] URL | Company | Role` format. Deduplicate by canonical URL and never add internships, social recruiting, expired pages, or unverified campus roles.
- The daily Loop may add verified candidates to the pipeline, but it must not run automatic evaluation, generate an application record, change a tracker status, send a message, or submit an application. Those remain explicit user actions inside up.
...
1. Find current, source-backed autumn-recruiting or campus-recruiting full-time opportunities for new graduates.
   - Include only roles explicitly confirmed as 秋招、校园招聘、应届生全职、管培生、产培生, or an equivalent graduate full-time program.
   - Exclude every internship, including 日常实习、暑期实习、留用实习, and Internship. Also exclude social-recruiting roles and roles whose campus/full-time status cannot be verified.
...
2. Build a source-linked interview experience library for every role in the workspace.
...
Every section contains questions. Every question contains a short synthesis and one or more independently sourced answer units. Each answer unit must point to the source IDs that support it. Use question wording that the user can directly practice, such as “一面通常会问什么？” or “为什么选择京东 TET 管培？”
  ...
  "updates": [
    {
      ...
      "type": "sync, opportunity, interview, source, or process",
...
- Before replacing the file, compare the new result with the previous valid `intelligence.json`. Append concise update records for added/removed opportunities, recruiting-status changes, new interview questions or sources, and changed process nodes or time ranges. Always append one `sync` record for the completed run, even when there is no substantive change. Preserve prior updates, deduplicate by `id`, sort newest first, and retain at most 100.
